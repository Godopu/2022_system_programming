...
int main() {
  ...
  shms = makeShms(1234, 1235, "programAB_sem");
  if (shms == NULL) {
  ...
  while (1) {
    ...
    retValue = readLine(sendBuf, BUF_SIZE);
    if (retValue < 0) {
      perror("read string from stdin error");
    }

    // send the line to peer (programB)
    writeToShm(shms, sendBuf, strlen(sendBuf));

    // receive the message from peer (programB)
    retValue = readFromShm(shms, recvBuf);
    if (retValue < 0) {
      perror("read message from shared memory error");
    }

    // if 'end' is entered from console, then break
...
void writeToShm(shmaddrs *shms, char *msg, int length) {
  ...
  *lengthField = length;
  ...
  strncpy(contentField, msg, length); // 주문서 작성
  contentField[length] = 0;

  sem_post(shms->s_sem); // 주문 전달
  printf("---> post %s\n", shms->s_sem_name);
